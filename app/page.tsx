...
const ScissorsIcon = () => (
  <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-8 sm:w-48">
...
const NailPolishIcon = () => (
  <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-8 sm:w-48">
...
export default function Home() {
  ...
  useEffect(() => {
    if (loadingComplete) {
      ...
      // Show content with slight delay after loading completes
      setTimeout(() => setShowContent(true), 100);
      
      // Show icons a bit later for better timing
  ...
  useEffect(() => {
    ...
    // Responsive initial rectangle size
    ...
    gsap.set(loadingRectRef.current, {
      ...
      x: '-50%',
      y: '-50%',
      backgroundColor: 'rgba(255, 214, 229, 0.9)',
      borderRadius: '16px'
    });
    ...
    .to(counterRef.current, { 
      fontSize: isMobile ? '10vh' : '18vh',
      opacity: 1,
      duration: 1 
    }, 0)
    .from(decorRefs.current, {
      opacity: 0,
      y: isMobile ? 10 : 20,
      stagger: 0.1,
      duration: 1.5
    ...
    <>
      ...
      {!loadingComplete && (
        <div 
          ...
          className="fixed inset-0 z-50 bg-[#1a0a12] flex items-center justify-center overflow-x-hidden"
        >
          {/* Expanding rectangle with inner decorations */}
          <div 
            ref={loadingRectRef} 
          ...
          >
            ...
            <p 
              ref={counterRef}
              className="text-[10vh] sm:text-[12vh] md:text-[18vh] font-bold text-pink-900 opacity-0 z-10"
              style={{ fontFamily: "'Warnes', cursive", textShadow: '0 0 15px rgba(255,255,255,0.7)' }}
            >
              {counter}%
            </p>

            {/* Custom SVG decorations */}
            <div ref={el => addDecorRef(el, 0)} className="absolute top-[15%] left-[15%] sm:top-1/4 sm:left-1/4 text-pink-700 opacity-70">
              <ScissorsIcon />
            </div>
            <div ref={el => addDecorRef(el, 1)} className="absolute bottom-[15%] right-[15%] sm:bottom-1/4 sm:right-1/4 text-pink-700 opacity-70">
              <LipstickIcon />
            </div>
            <div ref={el => addDecorRef(el, 2)} className="absolute top-[20%] right-[20%] sm:top-1/3 sm:right-1/3 text-pink-600 opacity-60">
              <NailPolishIcon />
            </div>
            <div ref={el => addDecorRef(el, 3)} className="absolute bottom-[20%] left-[20%] sm:bottom-1/3 sm:left-1/3 text-pink-600 opacity-60">
              <FlowerIcon />
            </div>
            <div ref={el => addDecorRef(el, 4)} className="absolute top-[10%] right-[10%] sm:top-1/5 sm:right-1/5 text-pink-500 opacity-50">
              <SparkleIcon />
            </div>
            <div ref={el => addDecorRef(el, 5)} className="absolute bottom-[10%] left-[10%] sm:bottom-1/5 sm:left-1/5 text-pink-500 opacity-50">
              <CrownIcon />
            </div>
          </div>
        </div>
      ...
      {showContent && (
        ...
        >
          ...
          <section className="py-16 px-8 bg-pink-50" data-aos="fade-up" data-aos-delay="600">
            <h2 className="text-4xl text-center mb-12 text-pink-700">
              Academy Milestones 
            </h2>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-6xl mx-auto text-center">
              {[
                ...
                >
                  ...
                  <p className="text-pink-800">{highlight.label}</p>
                </div>
              ))}
            </div>
          </section>
